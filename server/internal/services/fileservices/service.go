...
func New(fr FileRepo) *Service {
	return &Service{
		repo:       fr,
		uploadCh:   make(chan struct{}, maxUploadCons),   // Ограничение до 10 параллельных загрузок
		downloadCh: make(chan struct{}, maxDownloadCons), // Ограничение до 10 параллельных скачиваний
		listCh:     make(chan struct{}, maxListCons),     // Ограничение до 100 параллельных запросов списка
	}
}
...
func (s *Service) DownloadFile(req *v1.FileDownloadRequest, stream v1.FileUploadService_DownloadFileServer) error {
	select {
	case s.downloadCh <- struct{}{}:
		defer func() { <-s.downloadCh }()
